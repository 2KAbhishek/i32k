...
def main():
    ...
    if best_candidate:
        i3.command(f"[con_id={focused.id}] swap container with con_id {best_candidate.id}")
        return
    ...
    if len(leaves) == 2:
        ...
        if is_side_by_side:
            if direction in ("left", "right"):
                ...
                i3.command(f"[con_id={other.id}] focus; move down; [con_id={focused.id}] focus")
        else:
            # Stacked vertically
            if direction in ("up", "down"):
                ...
                i3.command(f"[con_id={other.id}] focus; move right; [con_id={focused.id}] focus")
        return

    # If 3+ windows and no adjacent neighbor, fallback to i3 native move
    i3.command(f"move {direction}")
